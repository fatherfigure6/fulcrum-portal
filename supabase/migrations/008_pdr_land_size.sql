-- =============================================================================
-- 008_pdr_land_size.sql
--
-- 1. Add land_min / land_max (NUMERIC m²) to request_pdr_details so a PDR
--    intake can record a land-size constraint.
-- 2. Replace the three PDR-creation RPCs (create_pdr_public, create_pdr_request,
--    create_pdr_staff) with versions that accept the two new params and persist
--    them. Old signatures are DROP-then-CREATE-OR-REPLACE so we don't leave
--    behind a stale overload that older clients could still hit.
-- =============================================================================

-- ── 1. Columns ────────────────────────────────────────────────────────────────

ALTER TABLE request_pdr_details
  ADD COLUMN IF NOT EXISTS land_min NUMERIC(10,2),
  ADD COLUMN IF NOT EXISTS land_max NUMERIC(10,2);
